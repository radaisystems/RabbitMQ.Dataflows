...
public class GzipTests
{
    ...
    [Fact]
    public async Task VerifyLengthCalculationAsync()
    {
        var lastFour = _compressedData.AsMemory(_compressedData.Length - 4, 4);
        var length = BitConverter.ToInt32(_compressedData.AsSpan(_compressedData.Length - 4, 4));
        var littleEndianLengthCalculation = (lastFour.Span[3] << 24) | (lastFour.Span[2] << 24) + (lastFour.Span[1] << 8) + lastFour.Span[0];
        var altLittleEndianLengthCalculation = (((((lastFour.Span[3] << 8) | lastFour.Span[2]) << 8) | lastFour.Span[1]) << 8) | lastFour.Span[0];
